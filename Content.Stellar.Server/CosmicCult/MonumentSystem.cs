...
public sealed class MonumentSystem : SharedMonumentSystem
{
    ...
    {
        ent.Comp.CurrentProgress = ent.Comp.TotalEntropy + cult.Comp.TotalCult * _config.GetCVar(STCCVars.CosmicCultistEntropyValue);
    }
    ...
    public void UpdateMonumentReqsForTier(Entity<MonumentComponent> monument, int tier)
    {
        ...
        var numberOfCrewForTier3 = Math.Round((double)cult.Comp.TotalCrew / 100 * _config.GetCVar(STCCVars.CosmicCultTargetConversionPercent)); // 40% of current pop

        switch (tier)
        {
            case 1:
                monument.Comp.ProgressOffset = 0;
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 / 2 * _config.GetCVar(STCCVars.CosmicCultistEntropyValue));
                break;
            case 2:
                monument.Comp.ProgressOffset = (int)(numberOfCrewForTier3 / 2 * _config.GetCVar(STCCVars.CosmicCultistEntropyValue)); //reset the progress offset
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 * _config.GetCVar(STCCVars.CosmicCultistEntropyValue));
                break;
            case 3:
                monument.Comp.ProgressOffset = (int)(numberOfCrewForTier3 * _config.GetCVar(STCCVars.CosmicCultistEntropyValue));
                monument.Comp.TargetProgress = (int)(numberOfCrewForTier3 * _config.GetCVar(STCCVars.CosmicCultistEntropyValue)); //removed offset; replaced with timer
                break;
        }
    }
